...
- **build** — the Builder produces an admitted world from manifest + pack. In a run, `OpenRangeRun` owns the dashboard event sink so pack loading, world generation, verification generation, admission, and snapshot creation are visible while the build runs. See [main doc](start_here.md).
...
- **get tasks** — the harness asks the world for the task instruction and entrypoints. See [Task](start_here.md#task).
...
- **verify** — the verifier runs on the world's final state and returns a structured outcome. See [Episode checks and rewards](start_here.md#episode-checks-and-rewards).
